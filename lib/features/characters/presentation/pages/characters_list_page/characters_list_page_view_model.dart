...
import '../../../../../core/router/router.dart';
import '../../widgets/common/app_warning_dialog_widget.dart';
import '../character_edit_page/character_edit_page_view_model.dart';

class CharactersListPageVM extends GetxController {
  ...
  Rx<SortType> sortType = SortType.no.obs;

  CharactersListPageVM(
  ...
  void editCharacter(CharacterEntity character, WidgetRef ref) {
    ref
        .read(characterEditPageViewModelProvider.notifier)
        .initNewCharacter(character);
  ...
  void onTapDeleteCharacter(BuildContext context, CharacterEntity character) async {
    _showWarning(context, character);
  }

  void goCharacterPlayPage(CharacterEntity character, WidgetRef ref) {
    ref
        .read(characterPlayPageVMProvider.notifier).initCharacter(character);

    RouterHelper.router.go(RouterHelper.characterPlayPath);
  }

  void updateCharacter(CharacterEntity character) {
    int index = characters.indexWhere((char) => char.localeId == character.localeId);
    if (index != -1) {
  ...
  void sortBy(String? value) {
    List<CharacterEntity> sortedCharacters = [...characters];

    SortType? newSortType;

    if (value == SortType.no.toLabel()) {
      sortedCharacters = _sortByLocalId(sortedCharacters);
      newSortType = SortType.no;
    } else if (value == SortType.name.toLabel()) {
      sortedCharacters = _sortByName(sortedCharacters);
      newSortType = SortType.name;
    } else if (value == SortType.createdAt.toLabel()) {
      sortedCharacters = _sortByCreatedAt(sortedCharacters);
      newSortType = SortType.createdAt;
    } else if (value == SortType.updatedAt.toLabel()) {
      sortedCharacters = _sortByUpdatedAt(sortedCharacters);
      newSortType = SortType.updatedAt;
    }
